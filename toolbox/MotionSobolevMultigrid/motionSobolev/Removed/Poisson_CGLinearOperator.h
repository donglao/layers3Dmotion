...
class Poisson_CGLinearOperator : public CGLinearOperator<double> {
	...
	public:
		Poisson_CGLinearOperator( int _XSize, int _YSize, int _GridSize, char *_region ) : 
		...
		void operator() ( const double *x, double *Ax ) {
			...
			for (int x2=0, p=0; x2<YSize; x2++) {
				for (int x1=0; x1<XSize; x1++, p++) {
					if (region[p]) {
						G1px = x1 < XSize - 1 && region[p+1] ? xc[p+1] - xc[p] : 0;
						G2px = x1 < XSize - 1 && region[p+1] ? yc[p+1] - yc[p] : 0;
						G1mx = x1 > 0         && region[p-1] ? xc[p-1] - xc[p] : 0;
						G2mx = x1 > 0         && region[p-1] ? yc[p-1] - yc[p] : 0;

						G1py = x2 < YSize - 1 && region[p+XSize] ? xc[p+XSize] - xc[p] : 0;
						G2py = x2 < YSize - 1 && region[p+XSize] ? yc[p+XSize] - yc[p] : 0;
						G1my = x2 > 0         && region[p-XSize] ? xc[p-XSize] - xc[p] : 0;
						G2my = x2 > 0         && region[p-XSize] ? yc[p-XSize] - yc[p] : 0;

						Ax[p] = -(G1px + G1mx + G1py + G1my );
						Ax[p+GridSize] = -(G2px + G2mx + G2py + G2my );
					}
				}
			}
