#ifndef CONJUGATEGRADIENT_NO_VIRTUAL_C
#define CONJUGATEGRADIENT_NO_VIRTUAL_C

#include "conjugateGradient_no_virtual.h"
//#include "params.h"

template <class T>
int conjugateGradient<T>::allocate(int Size, int Nblocks)
{
  if ( Size <= 0 ) return 0;
  N=Size;

  if ( Nblocks > 1 ) this->Nblocks = Nblocks;
...
void conjugateGradient<T>::computeSolution(double errorTol)
{
  ...
  if (poisson) Ax_Poisson(x, Ax); else Ax_Poisson_Dirichlet(x, Ax); // Ax = A(x)
  ...
  //  printf("rr start=%f, bb=%f\n", rr, bb);

  while (rr > tol && count < 2*numbActive ) { // 
    
//    A(p, Ap);                    // Ap = A(p)
    if (poisson) Ax_Poisson(p, Ap); else Ax_Poisson_Dirichlet(p, Ap);
