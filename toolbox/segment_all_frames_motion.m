function final_label = segment_all_frames_motion(org, labels, forwardflow, backwardflow, interval)
...
for i = 1:length(org)
    ...
    labels{i} = logical(labels{i});
end
%% solve initialization problem
if sum(sum(labels{round(interval/2)})) <(h*w/400);
    templabel = segment_first_16_frames_fixed(org, labels, forwardflow, backwardflow);
    final_label{round(interval/2)} = templabel{round(interval/2)};
end
%%
step = round(interval/4);
...
for index = 0:group-1
    ...
    [l, layers] =  segment_motion(o, l,f,b);
